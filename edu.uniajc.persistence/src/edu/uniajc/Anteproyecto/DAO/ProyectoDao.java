...
public class ProyectoDao {
 ...
 /*
    ...
    public ArrayList<Proyecto> getAllProyectos2(int codigo_usuario) {
        ArrayList<Proyecto> list = new ArrayList<>(0);
        try {

            PreparedStatement ps = null;

           final String SQL = "SELECT PROYECT.* FROM  TB_PROYECTO PROYECT  INNER JOIN TB_INTEGRANTES INTG   ON PROYECT.ID=INTG.ID_T_PROYECTO WHERE   INTG.ID_T_USUARIO = "+codigo_usuario+"";
            ps = this.DBConnection.prepareStatement(SQL);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                Proyecto proyecto = new Proyecto();
